...
namespace GnsMuhasebe.domain.Tests
{
    public class ProductTests
    {
        ...
        public void Constructor_CreateProduct_NormalData()
        {
            ...
            Product product = new Product(name, categoryId, desc, stock, purchasePrice, salePrice, barcode);
        ...
        {
            Assert.Throws<BusinessException>(() => new Product(invalidName, categoryId, desc, stock, purchasePrice, salePrice, barcode));
        }

        [Fact]
        public void Constructoor_NegativeSalePrice_ThrowException()
        {
            Assert.Throws<BusinessException>(() => new Product(name, categoryId, desc, stock, purchasePrice, -5, barcode));
        }

        [Fact]
        public void Constructoor_NegativePurchasePrice_ThrowException()
        {
            Assert.Throws<BusinessException>(() => new Product(name, categoryId, desc, stock, -5, salePrice, barcode));
        }
